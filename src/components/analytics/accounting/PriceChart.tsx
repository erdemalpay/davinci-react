...
const PriceChart = ({
  ...
  const nivoLineData = useMemo(() => {
    ...
    return chartConfig.series.map((serie: any) => ({
      id: serie.name,
      data: chartConfig.options.xaxis.categories.map(
  ...
  return (
    <Card className="shadow-none">
      <CardHeader
        ...
        shadow={false}
        color="transparent"
        className="flex flex-col sm:flex-row gap-4 rounded-none"
      >
        ...
        {customLegend && customLegend.length > 0 && (
          <div className="flex flex-wrap gap-2 sm:gap-3 sm:ml-auto items-center">
            {customLegend.map((item, index) => (
              <div
                key={index}
                className="flex items-center gap-2 px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium"
                style={{
          ...
          <ResponsiveLine
            ...
            sliceTooltip={({ slice }) => {
              ...
              const dataPointIndex =
                chartConfig?.options?.xaxis?.categories?.indexOf(xValue) ?? -1;

              // Sol kenarda mı kontrol et - piksel değerine göre
              const pointX = slice.points[0]?.x as number;
              const isNearLeftEdge = pointX < 150;

              // Tooltip pozisyon stili - sol kenarda sağa kaydır
              const tooltipPositionStyle: React.CSSProperties = isNearLeftEdge
                ? { transform: "translateX(80%)" }
                : {};

              // Eğer tooltipData varsa (CategorySummaryCompareChart'tan geliyorsa), detaylı tooltip göster
              if (
                tooltipData?.primaryPeriod &&
                tooltipData?.secondaryPeriod &&
                tooltipData.t &&
                dataPointIndex >= 0
              ) {
                try {
                  const primaryData =
                    tooltipData.primaryPeriod.data[dataPointIndex];
                  const secondaryData =
                    tooltipData.secondaryPeriod.data[dataPointIndex];
                  ...
                  if (!primaryData || !secondaryData) {
                    ...
                    const percentageChange =
                      secondaryValue !== 0
                        ? ((difference / secondaryValue) * 100).toFixed(1)
                        : "0";
                    ...
                    const formatDate = (dataItem: any, granularity: string) => {
                      if (granularity === "daily" && dataItem.date) {
                        ...
                        return `${t(monthKeys[monthIndex])} ${format(
                          date,
                          "yyyy"
                        )}`;
                      }
                    ...
                    const primaryDateLabel = formatDate(
                      primaryData,
                      tooltipData.granularity || "daily"
                    );
                    const secondaryDateLabel = formatDate(
                      secondaryData,
                      tooltipData.granularity || "daily"
                    );
