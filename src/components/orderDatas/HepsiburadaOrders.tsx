...
const HepsiburadaOrders = () => {
  ...
  const hepsiburadaOrdersPageDisabledCondition = useMemo(() => {
    return getItem(
      DisabledConditionEnum.ORDERDATAS_HEPSIBURADAORDERS,
      disabledConditions
    );
  }, [disabledConditions]);
  ...
  const rows = useMemo(() => {
    ...
    const allRows = orders
      ?.filter((order) => {
        if (
          order?.hepsiburadaOrderNumber !== null &&
          order?.hepsiburadaOrderNumber !== undefined &&
          order?.hepsiburadaOrderNumber !== ""
        ) {
  ...
  const columns = useMemo(
    () => [
      ...
      {
        key: t("Hepsiburada Order Number"),
        isSortable: true,
        correspondingKey: "hepsiburadaOrderNumber",
      },
      {
        key: t("Sales Channel"),
        isSortable: true,
  ...
  const rowKeys = useMemo(
    () => [
      ...
      { key: "hepsiburadaOrderNumber", className: "min-w-40 pr-2" },
...
export default HepsiburadaOrders;
