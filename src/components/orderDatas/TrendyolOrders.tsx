...
const TrendyolOrders = () => {
  ...
  const trendyolOrdersPageDisabledCondition = useMemo(() => {
    return getItem(
      DisabledConditionEnum.ORDERDATAS_TRENDYOLORDERS,
      disabledConditions
    );
  }, [disabledConditions]);
  ...
  const flatRows = useMemo(() => {
    ...
    const allRows = orders
      ?.filter((order) => {
        if (
          order?.trendyolOrderId !== null &&
          order?.trendyolOrderId !== undefined &&
  ...
  const rows = useMemo(() => {
    ...
    flatRows
      ...
      ?.forEach((row: any) => {
        const key = row?.trendyolOrderId as string;
  ...
  const columns = useMemo(
    () => [
      ...
      {
        key: t("Trendyol Order Number"),
        isSortable: true,
        correspondingKey: "trendyolOrderNumber",
      },
      {
        key: t("Sales Channel"),
        isSortable: true,
  ...
  const rowKeys = useMemo(
    () => [
      ...
      { key: "trendyolOrderNumber", className: "min-w-40 pr-2" },
